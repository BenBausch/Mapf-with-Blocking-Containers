...
class TAstar():
    ...
    def find_path(self):
        ...
        pp_counter = 1
        for g in self.goals:
            sub_path = Astar(self.h, self.agent, self.container, self.constrains, g, new_start, start_time).find_path()
            ...
            if pp_counter == 1:
                #agent moves alone
                #at the last step the agent arrives at the container so none is wrong
